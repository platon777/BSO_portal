...
export class MySubClassedDexie extends Dexie {
  ...
  constructor() {
    ...
    this.version(3).stores({
      personnes: 'id_personne, code_client, code_client_ancien, pseudo, lieu_de_travail, occupation, geocode, prenom, nom, piece_identification, email, numero_telephone, adresse, sexe, date_naissance, nif_cin, photo_identification, date_creation, statut, created_at, &unique_id, id_plan, montant, created_by, updated_by, updated_at',
      comptes_epargne: 'id_compte_epargne, id_personne, no_compte, no_compte_ancien, id_plan, solde_actuel, fonds_garantie, statut, date_creation, succursale, duree, person_allowed, piece_identification_allowed, nif_cin_allowed, photo_allowed, created_at, created_by, updated_by, updated_at',
      comptes_credit: 'id_compte_credit, id_personne, no_compte, ancien_code, id_compte_epargne, montant_prete, taux_interet, paiement_journalier, duree_credit_mois, fonds_garantie, penalites, statut, date_creation, created_at, created_by, updated_by, paiement_rembourse, montant_deja_paye_manuellement, updated_at',
    ...
    // Version 5: Add legacy code fields for migration from old system
    ...
    // Version 6: Add manual paid amount field in credit accounts
    ...
    // Version 7: Add update fields from feedback A/B/C
    this.version(7).stores({
      ...
      comptes_credit: 'id_compte_credit, id_personne, no_compte, ancien_code, id_compte_epargne, montant_prete, taux_interet, paiement_journalier, duree_credit_mois, fonds_garantie, penalites, statut, date_creation, date_debut, date_fin, created_at, created_by, updated_by, paiement_rembourse, montant_deja_paye_manuellement, updated_at',
      ...
      return tx.table('transactions_epargne').toCollection().modify(item => {
        if (item.frais_auto === undefined) {
          item.frais_auto = 0;
        }
        if (item.remise_client === undefined) {
          item.remise_client = 0;
        }
      });
    });

    // Version 8: Add occupation categorization fields (secteur, activite, etc.) and FA transaction type
  ...
  async deleteRecord(
    ...
    await this.transaction('rw', table, this.syncQueue, async () => {
      ...
      await this.addToSyncQueue('delete', tableName, pk, { [table.schema.primKey.name]: pk });
    });
  }
