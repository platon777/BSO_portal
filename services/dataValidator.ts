...
export const validateSyncItem = (item: SyncQueueItem): ValidationResult => {
  ...
  if (item.data) {
    switch (item.table) {
      case 'personnes':
        validatePersonne(item.data, errors);
        break;
      case 'comptes_epargne':
        validateCompteEpargne(item.data, errors);
        break;
      case 'comptes_credit':
        validateCompteCredit(item.data, errors);
        break;
      case 'transactions_epargne':
        validateTransactionEpargne(item.data, errors);
        break;
      case 'transactions_credit':
        validateTransactionCredit(item.data, errors);
        break;
    }
  }
...
const validatePersonne = (data: any, errors: string[]) => {
...
const validateCompteEpargne = (data: any, errors: string[]) => {
...
const validateCompteCredit = (data: any, errors: string[]) => {
  if (!data.id_personne) errors.push('ID personne is required');
...
const validateTransactionEpargne = (data: any, errors: string[]) => {
  if (!data.id_compte_epargne) errors.push('ID compte épargne is required');
...
const validateTransactionCredit = (data: any, errors: string[]) => {
